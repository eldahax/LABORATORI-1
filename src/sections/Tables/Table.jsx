...
export default function Table() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setAlert({
        show: true,
        message: err.response?.data?.error || "Failed to complete delete routing",
        type: "error",
      });
    } finally {
  ...
  const handleEditUserSuccess = () => {
    setAlert({ show: true, message: "User context variables saved cleanly!", type: "success" });
    fetchUsers();
  };
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return firstName.includes(lowerSearch) || lastName.includes(lowerSearch) || email.includes(lowerSearch);
  });
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl mb-8 overflow-x-auto">
      ...
      <EditUserModal show={editUserOpen} userId={editUserId} onClose={() => { setEditUserOpen(false); setEditUserId(null); }} onSuccess={handleEditUserSuccess} />
        ...
        <div className="flex w-full min-h-screen mt-[50px]">
          ...
          <div className="w-3/4 p-10 ml-[25%]">
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md mb-8 overflow-x-auto">
             <h1 className="text-lg sm:text-xl font-bold text-[#0F766E] mb-6">USERS</h1>
                  ...
                  {filteredUsers.map((user) => (
                    <tr key={user.user_id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="flex gap-2 py-3">
                        <button onClick={() => { setSelectedId(user.user_id); setConfirmOpen(true); }} className="text-red-500 hover:bg-red-500 hover:text-white px-3 py-1 rounded-lg transition cursor-pointer">Delete</button>
                        <button onClick={() => { setEditUserId(user.user_id); setEditUserOpen(true); }} className="text-green-600 hover:bg-green-500 hover:text-white px-3 py-1 rounded-lg transition cursor-pointer">Update</button>
                      </td>
                    </tr>
                  ))}
                  {filteredUsers.length === 0 && (
                    <tr><td colSpan="6" className="text-center py-8 text-gray-500">No matching users found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
