...
export default function TableRoom() {
  ...
  const handleDeleteD = async (department_id) => {
    try {
      ...
      if (res.ok) {
        setDepartments((prev) =>
          prev.filter(
            (dep) => dep.department_id !== department_id
          )
        );
  ...
  const filteredRooms = rooms.filter((room) => {
    ...
    const department = room.Department?.department_name?.toLowerCase() || "";

    const search = roomSearch.toLowerCase();

    return (roomName.includes(search) || department.includes(search));
  });

  const filteredDepartments = departments.filter((dep) => {
    const departmentName = dep.department_name?.toLowerCase() || "";

    return departmentName.includes(
      depSearch.toLowerCase()
    );
  });
